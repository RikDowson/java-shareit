...
public class BookingServiceImpl implements BookingService {
    ...
    private void validateUpdateBookingStatus(User user, Item item, Booking booking) {
        if (!user.getId().equals(item.getOwner().getId())) {
            throw new NotFoundException("Пользователь не может изменить статус бронирования");
        }
        if (booking.getStatus().equals(BookingStatus.APPROVED)) {
            throw new ValidationException("Статус бронирования уже установлен на APPROVED");
        }
        if (booking.getStatus().equals(BookingStatus.REJECTED)) {
            throw new ValidationException("Статус бронирования уже установлен на REJECTED");
        }
    }

    private void validateNewBooking(User user, Item item, Booking booking) {
        if (user.getId().equals(item.getOwner().getId())) {
            throw new NotFoundException("Пользователь не может забронировать свой товар");
        }
